...
for a in range(0,5):
    ...
    #Y_pred_NN = np.round(modelNN.predict(X_test)).astype(int)
    ...
    falseNegs = pd.DataFrame((SVM_fn, LR_fn, GNB_fn), ('svm', 'lr', 'GNB'))
    ...
    #Combine in ensemble 
    Y_pred = VotingClassification(estimators=[('svm', modelSVM), ('lr',modelLR), ('GNB', modelGNB)], voting='hard', weights=(weight_SVM,weight_LR,weight_GNB), n_jobs=None, flatten_transform=True)
